...
#include<bits/stdc++.h>
using namespace std;
// now we need to find out the element whose arrival time is greater than the store time
// and priority is maximum and busttime is not equals to zero
...
{
    //cout<<"index is "<<temp.index<<"arrival time"<<temp.arrivaltime<<"busttime"<<temp.busttime<<"priority"<<temp.priority<<endl;
}
int findthelowestnode(struct data* node,int countp,int storetime)
{
    ...
    for(i=1;i<=countp;i++)
    {
        ...
        if(flag==1)
        {
            if(node[i].priority>temp.priority&&(node[i].busttime!=0)&&node[i].arrivaltime<=storetime)
            {
              temp=node[i];
            }
            else if(node[i].priority==temp.priority&&node[i].busttime!=0&&node[i].arrivaltime<temp.arrivaltime)
            {
                temp=node[i];
            }
        }
...
int main()
{
    ...
    for(i=1;i<=countp;i++)
    {
        ...
        printf("Enter the PRIORITY  of the   ");
        printf("%d ",i);
        printf(" th process");
        printf("\n");
        scanf("%d",&node[i].priority);
        busttime[i]=node[i].busttime;
        node[i].index=i;
    }
    printf(" NOW THE ANS OF THE SCHEDULING PROBLEM ACCORDING TO THE PRIORITY IS \n");
