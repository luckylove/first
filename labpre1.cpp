...
//   &&&&&&&&&&&&&&&&& round robin &&&&&&&&&&&&&&&&&&&&&&
/*
Round robin scheduling algorithm is used to schedule process fairly each job a time slot or quantum and teh interrupting teh job if it is not completed by tan teh job come after teh other job which are arrived in teh quantum time that make these scheduling fairly
...
RR scheduling is also no as Time slicing scheduling
Advantages:

Each process is served by Cpu for a fixed time so priority is same for each one
Starvation is not occur coz of cyclic nataure.
Disadvantages:
...
int main()
{
    ...
    printf(" NOW THE ANS OF THE SCHEDULING PROBLEM ACCORDING TO THE ROUND ROBIN IS \n");
